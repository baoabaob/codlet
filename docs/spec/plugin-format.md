...
Combined entries share the logical plugin ID, registration, enabled preference and generation. Ordinary Host readiness precedes that package's Renderer activation. A dedicated [Native launch entry](host.md) instead runs only its bounded launch phase and creates no ordinary Host readiness dependency. Requirements use exact `{name, api, scope}` descriptors; duplicates/conflicts, missing providers and actual entry dependency cycles are rejected. In particular, an ordinary Host waiting on its own dependent Renderer creates a cycle. Core currently executes Runtime and Target scopes; declaring another scope does not create its lifecycle. See [RPC](rpc.md).
