...
状态：2026-09-20，八组服务的首版实现已落盘。本文保留架构设计与扩展验收目标，**不能将下面所有建议方法和理想验收项视为现有 API**。实际方法、限额、生命周期和平台限制以 [CORE_SERVICES.md](CORE_SERVICES.md) 及公开 TypeScript 契约为准。首版使用代次内任务记录、有界文件原子写入、非递归变化提示及原生基础通知；大文件写流、跨 Core 重启任务日志、通知自定义操作按钮仍是扩展方向。平台为 Windows 和 macOS；Linux、安装包及发布不在本轮范围。
