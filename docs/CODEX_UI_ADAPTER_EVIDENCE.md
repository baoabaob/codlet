...
The neutral active fallback deliberately avoids pairing the system Highlight background with CanvasText. Highlight is used only as the focus-outline fallback. System fallback appearance still follows the document's color-scheme settings.
...
- `node --test tests/*.test.mjs`: 35 passing tests including the existing runtime, GUI and local-plugin example suites at the baseline.
