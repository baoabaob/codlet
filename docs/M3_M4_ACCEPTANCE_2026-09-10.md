...
2026-09-10 当时的原生新建任务及冷恢复因 `features.thread_tools` 被以 `-32600` 拒绝，本轮历史验收采用最小创建/恢复请求准备专用任务。**2026-09-11 更正：该拒绝来自测试协调器额外的严格配置模式，不应归为上游缺陷。** 后续补齐专用后端配置并适配新版后，原生新建与冷恢复已通过；见[兼容修复与补验](DESKTOP_COMPATIBILITY_2026-09-11.md)。本页原始证据仍代表修复前范围。
