...
2026-09-12 补充：独立 fileChange 审批与 M0/M1 的含义、适用条件和手测步骤见[剩余审批与发布验收指南](REMAINING_ACCEPTANCE_MANUAL_2026-09-12.md)。已知发布缺陷和集中自动采样另行处理，不要求重复执行本页已完成用例。

2026-09-13：官方客户端升级后出现的“Start-TestClient.cmd 无反应”已修复，原入口已更新至核验过的新版本。失败时会保留错误窗口，见[启动修复说明](TEST_CLIENT_STARTUP_REPAIR_2026-09-13.md)。
