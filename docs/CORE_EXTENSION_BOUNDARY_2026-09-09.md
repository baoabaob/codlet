...
状态：2026-09-09 用户已确认，并通过侧边讨论同步到主任务。本文约束 M2–M4 及后续功能扩展。2026-09-10 的实现证据见 [M2 验收记录](M2_ACCEPTANCE_2026-09-10.md)与 [M3/M4 验收记录](M3_M4_ACCEPTANCE_2026-09-10.md)；M0/M1 保留尚未关闭的正式门禁。现行开发流程见 [Host 开发说明](HOST_DEVELOPMENT_2026-09-10.md)。

2026-09-11 补充确认：插件导入与社区分发继续沿用现有内核、授权与生命周期。Codlet 提供简单的导入和管理，GitHub/社区目录负责发现、版本与维护信息，当前不建设独立 Market。UI/Adapter 扩展和跨平台工作包见 [后续开发计划](NEXT_DEVELOPMENT_PLAN_2026-09-11.md)。
...
[raw-m2](../examples/raw-m2/README.md)用单一 Host 和公开 CDP 自行实现 binding、注入、新文档恢复与消息通路；其真实 JS 夹具不构造 RendererRuntime/TargetController，也不装载官方功能插件。正常清理由示例归还自身页面资源，Core 另行退休跟踪的 raw session；故障清理不会被描述为任意页面副作用回滚。后续可选 Desktop Adapter 已在 M3/M4 候选中交付，其私有映射继续留在目录包内。
