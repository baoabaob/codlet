...
`add` without `--trust` only inspects the directory and exits nonzero without registering it. Plugins that request permissions additionally need explicit `--grant ui.dom` and/or `--grant runtime.manage`. The example needs no extra grants. Removal preserves its files. Registration/grant edits keep existing enablement preferences; the next launch or an explicit enable/reload uses the updated authorization record. With `--watch`, changes to an already-loaded plugin's grants at the same path trigger reload validation. Schema 1 configurations remain readable; explicit saves migrate to schema 2 with the new `localPlugins` map.
