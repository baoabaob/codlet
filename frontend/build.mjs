  ...
  setup(b) {
    ...
    b.onLoad({filter: /[\\/]useEscCloseStack\.js$/}, async args => {
      ...
      const changes=[
        ...
        ['const id = useId();','const id = useId(); const scope = useContext(PortalContainer);'],
      ...
      return {contents:'import { PortalContainer, ownedEscape } from '+JSON.stringify(resolve(base,'src/portal-context.jsx'))+';\n'+source,loader:'js'};
    });
    b.onLoad({filter: /[\\/]react-use-escape-keydown[\\/]dist[\\/]index\.mjs$/}, async args => {
      let source=await readFile(args.path,'utf8');
      const changes=[
        ['const onEscapeKeyDown = useCallbackRef(onEscapeKeyDownProp);','const onEscapeKeyDown = useCallbackRef(onEscapeKeyDownProp); const scope = React.useContext(PortalContainer);'],
      ...
      return {contents:'import { PortalContainer } from '+JSON.stringify(resolve(base,'src/portal-context.jsx'))+';\n'+source,loader:'js'};
    });
  },
};
...
for (const [entry, destination] of [['src/adapter/entry.js','bundled/codex-ui-adapter/dist/renderer.js'],['src/codlet/app.jsx','bundled/codlet/dist/renderer.js'],['src/examples/ui-controls.jsx','examples/ui-controls/renderer.js'],['src/examples/desktop-m3-m4.jsx','examples/desktop-m3-m4/renderer.js']]) {
  const result=await build({absWorkingDir:base,entryPoints:[entry],bundle:true,write:false,metafile:true,format:'cjs',platform:'browser',jsxFactory:'h',jsxFragment:'React.Fragment',target:'chrome130',minify:false,legalComments:'inline',define:{'process.env.NODE_ENV':'"production"'},loader:{'.css':'text'}});
...
const licenseText=('Generated from the packages contributing code/styles to the UI bundles.\n\n'+[...notices].sort(([a],[b])=>a.localeCompare(b)).map(([,text])=>text).join('\n\n------------------------------------------------------------\n\n')+'\n').replace(/[ \t]+$/gm,'');
