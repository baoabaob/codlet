...
artifacts.set(resolve(root,'runtime/host-traffic-bundle.cjs'),'// Generated by frontend/build.mjs from runtime/host-traffic.cjs; includes ws 8.21.3 (MIT).\n'+trafficResult.outputFiles[0].text);
collectPackages(trafficResult.metafile);
const trafficWorker=await build({absWorkingDir:base,entryPoints:[resolve(root,'runtime/traffic-worker-entry.cjs')],bundle:true,write:false,metafile:true,format:'cjs',platform:'node',target:'node24',minify:false,legalComments:'inline',nodePaths:[resolve(base,'node_modules')]});
artifacts.set(resolve(root,'runtime/traffic-worker-bundle.cjs'),'// Generated by frontend/build.mjs from runtime/traffic-worker-entry.cjs; includes ws 8.21.3 (MIT).\n'+trafficWorker.outputFiles[0].text);
collectPackages(trafficWorker.metafile);
