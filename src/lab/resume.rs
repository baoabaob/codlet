...
impl ResumeEvidence {
    ...
    fn parse(bytes: &[u8], root: &Path, package: &str, version: &str) -> Result<Self, LabError> {
            ...
            match row["event"].as_str() {
                Some("lab_opened" | "prepared") => {
                    if detail["root"]
                        ...
                        || detail["package_full_name"] != package
                        || detail["package_version"] != version
        ...
        if !header || !((child.is_some() && exited && reaped) || (child.is_none() && no_child)) {
            return Err(blocked(
...
fn check_file(file: &File) -> Result<(), LabError> {
