...
pub(super) async fn fetch(
    ...
    if request.url.len() > 8192 {
        return Err(invalid("fetch URL exceeds 8192 bytes"));
    }
    let url = reqwest::Url::parse(&request.url)
        .map_err(|_| invalid("fetch requires an absolute HTTP(S) URL"))?;
    ...
    {
        return Err(invalid(
            "fetch URL must use HTTP(S), without credentials or fragment",
        ));
    }
