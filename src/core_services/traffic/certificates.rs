...
impl Certificates {
    pub(super) fn new() -> Result<Self> {
        ...
        params.distinguished_name.push(DnType::CommonName, "Codlet private launch traffic");
        let issuer = CertifiedIssuer::self_signed(params, KeyPair::generate().map_err(cert_error)?).map_err(cert_error)?;
        Ok(Self { issuer, leaves:BTreeMap::new() })
    }
    pub(super) fn pem(&self) -> String { self.issuer.pem() }
    pub(super) fn leaf(&mut self, hostname: &str) -> Result<Value> {
        let now = Instant::now();
        self.leaves.retain(|_,(expires,_)|*expires > now);
        if let Some((_,value))=self.leaves.get(hostname) { return Ok(value.clone()); }
        if self.leaves.len() >= 64 { return Err(error("resource_limit", "launch certificate limit reached")); }
        ...
        self.leaves.insert(hostname.into(),(now+Duration::from_secs(12*60*60),value.clone()));
        Ok(value)
    }
}
fn cert_error(_: rcgen::Error) -> ServiceError { error("certificate_unavailable", "cannot prepare the private launch certificate") }
