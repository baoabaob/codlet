...
export const uiSource=readFileSync(new URL('../../bundled/runtime/ui.js',import.meta.url),'utf8');
const i18nSource=readFileSync(new URL('../../bundled/runtime/i18n.js',import.meta.url),'utf8');
...
export function uiFixture({locale='en',request=async()=>({plugins:[]})}={}){
  ...
  const control=label=>[...document.querySelectorAll('[aria-label]')].find(el=>el.getAttribute('aria-label')===label) ?? document.getElementById([...document.querySelectorAll('label[for]')].find(el=>el.textContent===label)?.htmlFor) ?? undefined;
  return {dom,window,document,errors,context,calls,overrides,load,control,
