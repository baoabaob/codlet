...
function locateHost() {
  ...
  if (typeof navigator.push !== "function" || typeof navigator.replace !== "function" || typeof navigator.location?.pathname !== "string" || navigator.location.pathname.startsWith("/avatar-overlay"))
...
async function activate(context) {
  ...
  session.ready = (async () => {
    ...
    while (current === session) {
      try {
        ...
        session.navigation = createNavigation(context, native, locateHost());
  ...
  session.ready.catch((error) => {
    if (current === session) context.reportDiagnostic?.({ code: error.code || "ui_unavailable", message: error.message });
  });
